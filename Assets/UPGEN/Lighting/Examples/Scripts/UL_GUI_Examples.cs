...
public class UL_GUI_Examples : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (rendererData)
        {
            var renderFeature = rendererData.rendererFeatures.OfType<UPGEN_Lighting>().FirstOrDefault();
            if (renderFeature != null) renderFeature.intensity = 1;
        }
    ...
    private void OnGUI_Tools()
    {
        GUILayout.BeginArea(new Rect(0, 0, 200, Screen.height));
        {
            ...
            if (rendererData)
            {
                ...
                {
                    var v = renderFeature.intensity;
                    var nv = UL_GUI_Utils.Slider("Intensity", v, 0, 2);
                    if (nv != v) renderFeature.intensity = nv;
                }
            }
